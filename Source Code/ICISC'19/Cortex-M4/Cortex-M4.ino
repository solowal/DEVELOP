...
extern void __attribute__ ((noinline, naked)) enc_asm(u8* input, u8* round_key) ;
extern void __attribute__ ((noinline, naked)) dec_asm(u8* input, u8* round_key) ;
...
/**/
  ...
  for(int i=0;i<136;i++){
    Serial.println(round_key[i], HEX);
  }
 /* */
  Serial.println("Result of HIGHT encryption");
  for(int i=0;i<8;i++){
  ...
  enc_asm(input,round_key);
  Serial.println("Result of HIGHT encryption"); 
  for(int i=0;i<8;i++){
    Serial.println(input[i], HEX);
  }
  
  dec_asm(input,round_key);
  Serial.println("Result of HIGHT encryption"); 
  for(int i=0;i<8;i++){
    Serial.println(input[i], HEX);
  }
